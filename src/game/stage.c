...
/// For Windows version
static const int COL_DATA[] = {10,1,7,0,11,15,15,15,18,0,0,18,6,6,6,6,
4,0,2,0,6,17,17,15,18,18,18,18,0,6,0,6,
14,5,12,0,0,0,0,0,0,9,0,8,10,1,7,18,
15,14,5,12,0,5,5,0,0,0,6,0,4,0,2,18,
14,5,12,1,0,0,14,5,12,14,3,12,9,3,8,3,
...
static void tilemap_collisions(PLAYER* pl, float tm)
{
    ...
    for(layer=0; layer < map->layerCount; layer ++)
    {
        for(y = sy; y <= ey; y++)
        {
            for(x = sx; x <= ex; x++)
            {
                tid = COL_DATA [map->layers[layer] [y*map->w + x] -1];
